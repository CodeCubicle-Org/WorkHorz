...
bool http_routing::addPathResource(const std::string& path) {
  ...
  if (!uRes) {
    std::cerr << "Error: Path is not valid " << uRes.error().message()
              << std::endl;
    return bRet;
  }
  ...
  if (pos == std::string::npos) {
    std::cerr << "Error: No '/' found in the path" << std::endl;
    return bRet;
  }
  if (pos == sPath.size() - 1) {
    ...
    std::string sfilename = sPath.substr(pos + 1, sPath.size()); // the part after the last slash / is the filename
...
std::optional<MMPathlist> http_routing::findPath(const std::string& fpath) {
  if (!fpath.empty()) {
    ...
    std::cerr << "Error: path is empty or could not be found: " << fpath << std::endl;
    return std::nullopt;
  }
}
